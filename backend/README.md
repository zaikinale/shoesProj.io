...
Используются **Jest**, **Supertest** и мок Prisma (`jest-mock-extended`). Конфигурация: [jest.config.cjs](jest.config.cjs). В CI переменные окружения задаются в workflow (в т.ч. `NODE_ENV=test`, `CI=true`).
